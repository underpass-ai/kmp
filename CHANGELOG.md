...
- **The first-impression plan is now an enforced product contract.** Every
  ...
  unclassified page, a former public tool name in a current page, or a tool
  count that disagrees with the binary. The public pitch is three narrow
  claims with three VHS recordings generated by one command, and CI reruns
  the live scenarios behind them. Finally, a matching plugin and engine can
  no longer be stale in silence: the session hook checks the latest release
  once a day, and `/kmp:setup` updates both halves together with one command.
