...
fn namespaced_memory(
    ...
    for entry in &memory.entries {
        ...
        for coordinate in &entry.coordinates {
            ...
            {
                return Err(ApplicationError::Validation(format!(
                    "memory entry coordinate dimension `{}` does not match declared kind `{expected_kind}` for scope `{}`",
    ...
    for relation in &memory.relations {
        ...
        let coordinate = if let Some(coordinate) = relation.coordinate.as_ref() {
            require_non_empty(
                &coordinate.dimension,
                "memory.relations[].coordinate.dimension",
            )?;
            require_non_empty(
                &coordinate.scope_id,
                "memory.relations[].coordinate.scope_id",
            )?;
            let scope_id = dimension_aliases
                .get(&coordinate.scope_id)
                .cloned()
            ...
            if !dimension_ids.contains(&scope_id) {
                return Err(ApplicationError::Validation(format!(
                    "memory relation coordinate references unknown dimension scope `{}`",
                    coordinate.scope_id
                )));
            }
            if let Some(expected_kind) = declared_dimension_kinds.get(&scope_id)
            ...
            {
                return Err(ApplicationError::Validation(format!(
                    "memory relation coordinate dimension `{}` does not match declared kind `{expected_kind}` for scope `{}`",
                    coordinate.dimension, coordinate.scope_id
                )));
            }
            validate_positive_optional(
                coordinate.sequence,
                "memory.relations[].coordinate.sequence",
            )?;
            validate_positive_optional(coordinate.rank, "memory.relations[].coordinate.rank")?;
            let mut coordinate = coordinate.clone();
            coordinate.scope_id = scope_id;
            Some(coordinate)
        } else {
