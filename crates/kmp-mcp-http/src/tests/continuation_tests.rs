...
async fn send(path: &std::path::Path, identity: Identity, action: &Value) -> Response {
    let app = router(AppState::new(
        config(Duration::from_secs(5)),
        KernelMcpServer::embedded(path).expect("server"),
...
async fn retained_read_cannot_bypass_raw_about_reference_or_dimension_grants() {
    ...
    for denied in [identity(&["kmp:read"]), wrong_about, wrong_ref, no_read] {
        assert_eq!(
            send(dir.path(), denied, &action).await.status(),
            StatusCode::FORBIDDEN
        );
    }
    let accepted = send(dir.path(), allowed.clone(), &action).await;
    ...
    assert_eq!(
        send(dir.path(), allowed.clone(), &action).await.status(),
        StatusCode::FORBIDDEN,
    ...
    assert_eq!(
        send(dir.path(), no_all, &action).await.status(),
        StatusCode::FORBIDDEN
    );
    let accepted = send(dir.path(), allowed, &action).await;
    assert_eq!(accepted.status(), StatusCode::OK);
