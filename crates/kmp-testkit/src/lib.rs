...
fn seed_bundle(case_id: kmp_domain::CaseId, role: &str) -> kmp_domain::KmpBundle {
    let role = kmp_domain::Role::new(role).expect("role must be valid");
    kmp_domain::KmpBundle::new(
        case_id.clone(),
        role.clone(),
        kmp_domain::BundleNode::new(
            case_id.as_str(),
            "capability",
        ...
        Vec::new(),
        Vec::new(),
        vec![kmp_domain::BundleNodeDetail::new(
            case_id.as_str(),
            format!(
                "bundle for node {} role {}",
                case_id.as_str(),
        ...
        kmp_domain::BundleMetadata::initial("0.1.0"),
    )
