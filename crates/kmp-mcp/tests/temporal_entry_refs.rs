...
fn query(reference: Value) -> Value {
    json!({"about":"project:seed-routing","move":"goto","at":{"time":"2026-09-10T10:00:00Z"},"axis":"observed","refs":[reference],"include":{"dependencies":true},"limit":{"entries":1},"budget":{"max_bytes":500000}})
}
#[tokio::test]
async fn chosen_entry_keeps_inclusive_cutoff_and_full_dependency_sources() {
    ...
    let focused = call(&server, "kmp_time", query(refs["role"].clone())).await;
    ...
    let canonical = call(&server, "kmp_time", all).await;
    for entry in extra {
        assert!(
            canonical["entries"]
                .as_array()
...
async fn refs_do_not_override_time_scope_or_hard_label_filters() {
    ...
    let absent = call(&server, "kmp_time", args).await;
    ...
    let undated = call(&server, "kmp_time", missing_clock).await;
...
async fn focus_precedes_limits_and_history_navigation_retains_the_ref_set() {
    ...
    let mut args = query(refs["role"].clone());
    args["at"] = json!({"ref":refs["boundary"]});
    let old = call(&server, "kmp_time", args).await;
...
async fn invalid_explicit_ref_sets_are_rejected_instead_of_reading_everything() {
    ...
    for refs in [
        ...
        args["refs"] = refs;
        assert_eq!(raw(&server, "kmp_time", args).await["isError"], true);
    }
}
...
async fn reference_focus_is_retained_by_pages_and_binds_the_cursor() {
    ...
    let server = KernelMcpServer::embedded(dir.path()).expect("embedded");
    let refs = seed(&server).await;
    let full = call(&server, "kmp_time", query(refs["role"].clone())).await;
    let mut args = query(refs["role"].clone());
    ...
    assert_eq!(raw(&server, "kmp_time", changed).await["isError"], true);
    let mut pages = vec![first];
    for _ in 0..100 {
        let page = call(&server, "kmp_time", next).await;
