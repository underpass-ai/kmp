...
/// Maps the existing temporal options at the MCP boundary.
pub(super) struct TemporalOptionsMapper;

impl TemporalOptionsMapper {
    pub(super) fn temporal_axis_from_arguments(arguments: &Value) -> Result<i32, String> {
        ...
        let axis = JsonFieldReader::optional_string_field(arguments, "axis", "axis")?;
    ...
    pub(super) fn temporal_cursor_from_arguments(
        ...
        let arguments = JsonFieldReader::object(arguments, "tool arguments")?;
        let cursor = JsonFieldReader::required_object_field(arguments, cursor_key, cursor_key)?;
        let ref_value =
            JsonFieldReader::optional_string_field(cursor, "ref", &format!("{cursor_key}.ref"))?;
        let time = JsonFieldReader::optional_timestamp_field(
            cursor,
            "time",
            &format!("{cursor_key}.time"),
        )?;
        let sequence = JsonFieldReader::optional_positive_u32_field(
            cursor,
            "sequence",
            &format!("{cursor_key}.sequence"),
        )?;
        let present = [
            ref_value
                .as_ref()
    ...
    pub(super) fn temporal_window_from_arguments(
        ...
        let Some(window) = JsonFieldReader::optional_object_field(
            JsonFieldReader::object(arguments, "tool arguments")?,
            "window",
            "window",
        )?
        else {
        ...
        Ok(Some(TemporalWindow {
            before_entries: JsonFieldReader::optional_u32_field(
                window,
                "before_entries",
                "window.before_entries",
            )?
            .unwrap_or_default(),
            after_entries: JsonFieldReader::optional_u32_field(
                window,
                "after_entries",
    ...
    pub(super) fn temporal_limit_from_arguments(
        ...
        let Some(limit) = JsonFieldReader::optional_object_field(
            JsonFieldReader::object(arguments, "tool arguments")?,
            "limit",
            "limit",
        )?
        ...
        Ok(Some(TemporalLimit {
            entries: JsonFieldReader::optional_positive_u32_field(
                limit,
                "entries",
                "limit.entries",
            )?
            .unwrap_or_default(),
            tokens: JsonFieldReader::optional_positive_u32_field(limit, "tokens", "limit.tokens")?
                .unwrap_or_default(),
        }))
    }
    ...
    pub(super) fn temporal_include_from_arguments(
        ...
        let Some(include) = JsonFieldReader::optional_object_field(
            JsonFieldReader::object(arguments, "tool arguments")?,
            "include",
            "include",
        )?
        ...
        let raw_refs =
            JsonFieldReader::optional_bool_field(include, "raw_refs", "include.raw_refs")?
                .unwrap_or(false);
        let dependencies =
            JsonFieldReader::optional_bool_field(include, "dependencies", "include.dependencies")?
                .unwrap_or(false);
        if dependencies
            && ["evidence", "relations"]
                .iter()
        ...
        Ok(Some(TemporalInclude {
            evidence: JsonFieldReader::optional_bool_field(
                include,
                "evidence",
                "include.evidence",
            )?
            .unwrap_or(false),
            relations: JsonFieldReader::optional_bool_field(
                include,
                "relations",
                "include.relations",
            )?
    ...
    pub(super) fn page_from_arguments(arguments: &Value) -> Result<Option<PageRequest>, String> {
        let Some(page) = JsonFieldReader::optional_object_field(
            JsonFieldReader::object(arguments, "tool arguments")?,
            "page",
            "page",
        )?
        ...
        Ok(Some(PageRequest {
            entries: JsonFieldReader::optional_positive_u32_field(page, "entries", "page.entries")?
                .unwrap_or_default(),
    ...
    pub(super) fn inspect_include_from_arguments(
        ...
        let raw =
            JsonFieldReader::optional_bool_field(include, "raw", "include.raw")?.unwrap_or(false);
        Ok(Some(InspectInclude {
            incoming: JsonFieldReader::optional_bool_field(
                include,
                "incoming",
                "include.incoming",
            )?
            .unwrap_or(true),
            outgoing: JsonFieldReader::optional_bool_field(
                include,
                "outgoing",
                "include.outgoing",
            )?
            .unwrap_or(true),
            details: JsonFieldReader::optional_bool_field(include, "details", "include.details")?
                .unwrap_or(true),
            raw,
        }))
    }
    ...
    pub(super) fn as_of_from_arguments(
        arguments: &Value,
    ) -> Result<Option<TemporalCursor>, String> {
        let arguments = JsonFieldReader::object(arguments, "tool arguments")?;
        let Some(cursor) = JsonFieldReader::optional_object_field(arguments, "as_of", "as_of")?
        else {
            return Ok(None);
        };
        let ref_value = JsonFieldReader::optional_string_field(cursor, "ref", "as_of.ref")?
            .filter(|value| !value.trim().is_empty());
        let time = JsonFieldReader::optional_timestamp_field(cursor, "time", "as_of.time")?;
    ...
    pub(super) fn interval_from_arguments(
        ...
        let arguments = JsonFieldReader::object(arguments, "tool arguments")?;
        let Some(interval) =
            JsonFieldReader::optional_object_field(arguments, "interval", "interval")?
        else {
            return Ok(None);
        };
        let start = JsonFieldReader::optional_timestamp_field(interval, "start", "interval.start")?;
        let end = JsonFieldReader::optional_timestamp_field(interval, "end", "interval.end")?;
...
mod tests {
    ...
    fn temporal_cursor_requires_exactly_one_position() {
        let error = TemporalOptionsMapper::temporal_cursor_from_arguments(
            &json!({
                "from": {
                    "ref": "claim:1",
                    "sequence": 1
    ...
    fn temporal_window_rejects_unsupported_seconds_bounds() {
        let error = TemporalOptionsMapper::temporal_window_from_arguments(&json!({
            "window": {
                "before_seconds": 60,
                "after_entries": 2
            }
    ...
    fn temporal_axis_is_explicit_and_closed() {
        assert_eq!(
            TemporalOptionsMapper::temporal_axis_from_arguments(&json!({"axis": "ingested"}))
                .expect("known axis"),
            TemporalAxis::Ingested as i32
        );
        assert!(
            TemporalOptionsMapper::temporal_axis_from_arguments(&json!({"axis": "effective"}))
                .is_err()
        );
    }
    ...
    fn inspect_include_only_narrows_fields_named_by_the_caller() {
        assert!(
            TemporalOptionsMapper::inspect_include_from_arguments(&json!({}))
                .expect("an absent include is valid")
                .is_none()
        );

        let include = TemporalOptionsMapper::inspect_include_from_arguments(&json!({
