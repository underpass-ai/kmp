...
/// Maps the existing trace seek options at the MCP boundary.
pub(super) struct TraceSeekOptionsMapper;

impl TraceSeekOptionsMapper {
    ...
    pub(super) fn from_arguments(
    ...
    ) -> Result<Option<TraceSeekOptions>, String> {
        ...
        Ok(Some(TraceSeekOptions {
            roles,
            same_labels: JsonFieldReader::optional_string_array_field(
                search,
                "same_labels",
                "search.same_labels",
            )?,
    ...
    fn endpoint(value: &Value) -> Result<TraceReferenceEndpoint, String> {
        ...
        let point = JsonFieldReader::object(value, "search.same_ref endpoint")?;
        if point.keys().any(|k| !["role", "at"].contains(&k.as_str())) {
            return Err("search.same_ref endpoint accepts only role and at".into());
        }
        let role = JsonFieldReader::optional_string_field(point, "role", "search.same_ref.role")?
            .ok_or("search.same_ref endpoint requires role")?;
        let at = JsonFieldReader::optional_string_field(point, "at", "search.same_ref.at")?
    ...
    fn step(value: &Value) -> Result<TraceRelationStep, String> {
        ...
        let s = JsonFieldReader::object(value, "search.seek relation")?;
        Ok(TraceRelationStep {
            rel: JsonFieldReader::optional_string_field(s, "rel", "search.seek.rel")?
                .ok_or("search.seek relation requires rel")?,
            direction: JsonFieldReader::optional_string_field(
                s,
                "direction",
                "search.seek.direction",
            )?
    ...
    fn role(value: &Value) -> Result<TraceSeekRole, String> {
        ...
        if let Some(s) = value.as_object() {
            ...
            role.name = JsonFieldReader::optional_string_field(s, "name", "search.seek.name")?
                .unwrap_or(role.name);
            ...
            if let Some(labels) = s.get("labels") {
                role.labels = JsonFieldReader::object(labels, "search.seek.labels")?
                    .iter()
                    .map(|(key, v)| {
                        let values = v
                            .as_array()
...
mod tests {
    ...
    fn reference_groups_accept_anchor_and_witness_points_without_numeric_positions() {
        ...
        let request = super::super::TraceRequestMapper::from_arguments(&value).expect("parsed");
        ...
        for point in [
            ...
            let refused = match super::super::TraceRequestMapper::from_arguments(&bad) {
                Err(_) => true,
                Ok(proto) => {
    ...
    fn shorthand_and_advanced_moves_compile_through_the_same_proto_boundary() {
        ...
        for change in [
            ...
            let mut bad = value.clone();
            bad["search"] = change;
            assert!(super::super::TraceRequestMapper::from_arguments(&bad).is_err());
        }
        let mut bad = value;
        bad["to"] = json!("t");
        assert!(super::super::TraceRequestMapper::from_arguments(&bad).is_err());
