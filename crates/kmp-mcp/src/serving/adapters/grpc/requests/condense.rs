...
use super::common::{object, optional_string_field};
...
pub(crate) fn condense_request_from_arguments(
    ...
    let record_digest = optional_string_field(source, "record_digest", "source")?
        .filter(|digest| !digest.trim().is_empty())
        .ok_or("source.record_digest identifies the exact body version this card stands for")?;
    let expect = object(
    ...
    Ok(CondenseRequest {
        ...
        source_content_hash: optional_string_field(source, "content_hash", "source")?
            .unwrap_or_default(),
