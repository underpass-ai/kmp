use super::common::{object, optional_u32_field};
use kmp_proto::v1beta1::{
    TraceMaterialSelectionOptions, TraceProofAlternative, TraceProofRequirement,
};
use serde_json::Value;

pub(super) fn arguments(value: &Value) -> Result<TraceMaterialSelectionOptions, String> {
    let s = object(value, "search.select")?;
    let bounded = |key: &str, default: Option<u32>, max: u32| {
        let v = optional_u32_field(s, key, &format!("search.select.{key}"))?
            .or(default)
    ...
    let groups = match s.get("groups") {
        ...
        Some(Value::Array(groups)) if groups.len() <= 8 => groups
            .iter()
            .map(|group| {
                let g = object(group, "search.select.groups[]")?;
                let weight =
                    optional_u32_field(g, "weight", "search.select.groups[].weight")?.unwrap_or(1);
...
mod tests {
    ...
    fn trace_material_parser_keeps_and_or_requirements_and_rejects_invalid_shapes() {
        ...
        for input in [
            ...
            assert!(arguments(&input).is_err(), "accepted {input}");
        }
    }
}
