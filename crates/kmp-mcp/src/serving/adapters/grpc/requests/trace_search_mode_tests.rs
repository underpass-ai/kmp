...
fn one_repair_removes_all_mixed_search_fields_before_parsing_their_bodies() {
    ...
    let error = super::arguments(&request).expect_err("mixed modes");
    for field in [
        "to",
        "search.follow",
        "search.select",
    ...
    let (_, targets, options) = super::arguments(&request).expect("one repaired request");
...
fn destination_options_still_parse_and_unknown_nested_fields_still_fail() {
    ...
    let (_, targets, options) = super::arguments(&request).expect("destination mode");
    assert_eq!(targets, ["t"]);
    let options = options.expect("search");
