    ...
    tool_definition_with_output(
        ...
        json!({
            ...
            "properties": {
                ...
                "page": page_schema("Whole items: trace, candidates/groups in seek, then proof objects/supports/gaps when requested."),
...
fn trace_output_schema() -> Value {
    let mut schema = output_object(json!({
        ...
        "seek": described("object", "Seed evidence selection: status compatible, ambiguous, review_required, missing_obligation, incompatible_obligations or partial. declared_obligations_complete describes path/witness constraints, independently of proof source availability, and is always false for contextual discovery; it never means answer completeness or unique identity. Reports missing_roles, work/cut counters, role order and complete candidate/group counts. Optional review_context is a new scoped Goto read, separate from mandatory page continuations. Source materialization is optional through search.proof; semantic inference is never performed."),
        ...
        "proof": described("object", "Present with search.proof. expansion_plan names the next batch of bodies to ask for, decided over the complete selection before this response was paginated, so a page that holds none of the pending refs still carries it; expand_bodies and expand_oversized_body are that plan as complete calls. They continue a suffix of the manifest order and never claim a selection is complete: join responses by ref against one manifest, and read every page before concluding a selection is exhausted. Counts missing refs/bodies, incomplete entries and unknown attachment clocks. complete_groups/incomplete_groups index the declared target requirements or seek groups and describe fetched sources, not truth, lifecycle status or semantic sufficiency; a group whose body was deferred, not requested or shown as a card is not fetched proof. Join all pages and retain seek review_required/unknown. stop_reason and body_bytes report joint materialization work; body_bytes counts canonical UTF-8 bytes actually loaded, never a body this response withheld. With a body option active it also carries manifest_id, delivery counts and, for a compact read, the card summary."),
        "expansion_refusal": nullable_described("object", "Present only when a named expansion was refused before any body was read. code is read_selection_changed, with the expected and actual manifest, or unknown_expansion_refs, with the refs named outside this selection. Either way the response has no object, no support and no text: a partly valid batch delivers nothing. Repeat the original read without proof_refs or expect_selection to obtain the current manifest."),
        "objects": described("array", "Shared selected entries and typed sources: exact available body, ref, kind, canonical snapshot status (not historical lifecycle), source metadata, source time, support clocks, admitted coordinates, hash/revision and has_body. Missing bodies are marked; summary text with has_body=false is not the full source."),
        "supports": described("array", "Eligible typed source-to-entry attachments with original arrows, why/evidence and clocks. Join through refs to objects and routes/candidates; a shared source is returned once."),
        "gaps": described("array", "Per-ref missing_ref, missing_body, incomplete_entry and clock_unknown reasons. A work cutoff is incomplete retrieval, not evidence of absence."),
    ...
    schema["properties"]["proof"]["properties"] = json!({
        "condense_candidates": super::trace_condense::schema()
    });
    schema
}
...
fn trace_search_schema() -> Value {
    ...
    "description":"Choose exactly one mode below. Known destinations use to and destination options; evidence discovery uses seek and witness joins, without to. Work limits and proof are shared search fields. as_of/interval/axis select time at call level; page/budget page the result, not the search. Same-about traversal.",
    "properties": {
        "proof":{"type":"boolean","default":false,"description":"Fetch selected path entry bodies and typed sources together in the search snapshot. Shares N/E with discovery; a cutoff leaves explicit gaps. Adds whole objects/supports/gaps to the paged proof. Response max_bytes does not bound store allocations."},
        ...
        "max_body_record_bytes":{"type":"integer","minimum":1,"description":"With proof:true, ceiling on stored record bytes of canonical bodies this response may carry. Body delivery only: discovery, ranking, which paths qualify and which refs are selected are unchanged. A deterministic sorted prefix is admitted; once the next present record does not fit, it and every later one are deferred with their exact requirement, never skipped and never reported absent. Omitted is the unbounded legacy materialization."},
        "proof_refs":{"type":"array","maxItems":64,"uniqueItems":true,"items":{"type":"string","minLength":1},"description":"With proof:true, load only these refs of the selected proof table, which must be inside it; a ref outside is refused rather than widening the selection. Present and empty is a descriptor-only view. Nonempty requires both max_body_record_bytes and expect_selection. Named refs affect body delivery only."},
        "expect_selection":{"type":"string","minLength":1,"description":"The proof.manifest_id returned by the read that named these refs. Compared before any body is read: an equal manifest means the chunks may be joined by ref; a different one is refused with no canonical or card text, because the store keeps only current bodies and an old graph joined to new text is not a proof anybody selected."},
        "compact":{"type":"object","additionalProperties":false,"required":["language"],"description":"With proof:true, show valid reader-authored cards as orientation, never fetched proof or a closed group. Absent, stale, post-cut or wrong-language cards keep their ref, status, descriptor and expansion price but no prose; no canonical fallback. Named proof_refs take precedence. For fresh compact reuse, omit proof_refs, expect_selection and the old page cursor; keep the graph selection.","properties":{"language":{"type":"string","minLength":1,"description":"Language the cards were written in. A card in another language is absent here, never substituted."}}}
    }});
    ...
    let work_fields = [
        ...
        "max_states",
        "proof",
        // Body delivery is shared by both modes: each materializes the same
        // proof table and each pays for the same bodies.
        "max_body_record_bytes",
        "proof_refs",
        "expect_selection",
        "compact",
