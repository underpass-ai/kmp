...
fn trace_search_schema() -> Value {
    ...
    "properties": {
        ...
        "compact":{"type":"object","additionalProperties":false,"required":["language"],"description":"Present reader-authored cards instead of the canonical bodies they still stand for. Presentation only: no path, no ranking and no proof count changes, and a node without a usable card keeps its place with its body. A card is orientation, never fetched proof, so it never closes a group.","properties":{"language":{"type":"string","minLength":1,"description":"Language the cards were written in. A card in another language is absent here, never substituted."}}}
    }});
