...
fn trace_output_schema() -> Value {
    output_object(json!({
        ...
        "proof": described("object", "Present with search.proof. Counts missing refs/bodies, incomplete entries and unknown attachment clocks. complete_groups/incomplete_groups index the declared target requirements or seek groups and describe fetched sources, not truth, lifecycle status or semantic sufficiency. Join all pages and retain seek review_required/unknown. stop_reason and body_bytes report joint materialization work; bytes are canonical UTF-8 body bytes, not transport or allocations."),
...
fn trace_search_schema() -> Value {
    ...
    let work_fields = ["max_nodes", "max_edges", "max_depth", "max_states", "proof"];
