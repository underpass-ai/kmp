...
pub(crate) fn curate_schema() -> Value {
    json!({
        ...
        "required": ["mode", "about"],
        "properties": {
            "mode": {"type": "string", "enum": ["review", "apply", "paths"], "description": "`review` reads, judges and freezes; it writes nothing. `paths` finds whole chains from `from` (to `to`, or as far as they go) over declared relations and steps Jev proposes; it writes nothing. `apply` writes the accepted items of a frozen review — a proposed path step included — through the writer's own review."},
