//! `kmp-mcp snapshot create|list|verify|read|merge` — the durability verbs.
//!
//! One concept: the operator's view of committed memory bundles. Every
//! subcommand answers with the same JSON receipt shape, which is why they are
//! written together rather than one file each.
