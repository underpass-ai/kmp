//! `kmp-mcp export|import` — moving the append-only event log between stores.
//!
//! One concept: the two halves of one transfer. They share an argument shape,
//! a store resolution and a receipt, and splitting them would put the two ends
//! of the same operation in two files.

use super::help::{looks_like_option, unknown_option};
use std::path::{Path, PathBuf};

pub(super) async fn run(command: &str, args: &[&str]) -> i32 {
