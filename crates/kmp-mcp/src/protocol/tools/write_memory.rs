//! `kmp_write_memory` — the validating writer.
//!
//! One concept: the writer's argument shape, the read audit it asks callers to
//! declare, and the receipt it answers with. The relation vocabulary it
//! advertises is projected from the domain by `relation_vocabulary`, so this
//! module describes the write and never restates the catalog.
...
use crate::protocol::schema::{described, output_object, string_array, string_schema};
...
pub(in crate::protocol) fn read_context_schema() -> Value {
    json!({
        "type": "object",
        "additionalProperties": false,
