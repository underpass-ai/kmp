...
use kmp_domain::{
    ...
    ContextPathNeighborhood, GraphNeighborhoodReader, KmpBundle, MemoryAboutIndexReader,
    ...
    ProjectionMutation, ProjectionWriter, RelationExplanation, RelationSemanticClass, Role,
    SnapshotSaveOptions, SnapshotStore,
};
use kmp_observability::quality_observers::NoopQualityObserver;
...
fn describe_mentions_bind_address() {
    let config = kmp_config::AppConfig {
        service_name: "kmp".to_string(),
        grpc_bind: "127.0.0.1:50054".to_string(),
        grpc_tls: kmp_config::GrpcTlsConfig::disabled(),
...
async fn grpc_server_application_accessors_return_callable_services() {
    ...
    let update = server
        .command_application()
        .update_context(kmp_application::UpdateContextCommand {
            root_node_id: "node-123".to_string(),
            role: "developer".to_string(),
...
async fn command_service_accepts_update_context() {
    ...
    let service =
        CommandGrpcServiceV1Beta1::new(Arc::new(CommandApplicationService::new(Arc::new(
            kmp_application::UpdateContextUseCase::new(event_store, "0.1.0"),
        ))));
...
async fn command_service_returns_aborted_on_revision_conflict() {
    ...
    let status = service
        .update_context(Request::new(UpdateContextRequest {
            ...
            changes: vec![],
            metadata: None,
            precondition: Some(kmp_proto::v1beta1::RevisionPrecondition {
...
fn helper_mappers_cover_versions_errors_and_trim_logic() {
    ...
    let invalid_argument = map_application_error(ApplicationError::Domain(
        kmp_domain::DomainError::EmptyValue("root_node_id"),
    ));
...
fn sample_bundle(root_node_id: &str, role: &str, summary: &str) -> KmpBundle {
...
fn sample_node_neighborhood(node_id: &str, status: &str) -> NodeNeighborhood {
    NodeNeighborhood {
        root: kmp_domain::NodeProjection {
            node_id: node_id.to_string(),
            node_kind: "task".to_string(),
