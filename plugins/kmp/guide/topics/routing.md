...
| Enumerate a period: yesterday, since, before/after, what changed, current/latest/recent state, why now, or a release/decision window | `kmp_time` with `move` `forward` or `rewind` and a direct `interval`; `goto` or `near` to position or zoom |
