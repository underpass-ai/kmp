...
`kmp_relabel`, `kmp_condense`, `kmp_summaries_audit` and `kmp_curate`.
...
(`time`, whose `move` is `goto`, `near`, `rewind` or `forward`), audit
(`trace`, `inspect`), and write (`write_memory`, `relabel` for the labels of a
memory that exists, low-level `ingest`, `summaries_audit` to see which memories
an English question cannot reach before writing their renderings, `curate` to
review missing and doubtful relations with Jev before declaring them yourself), with the
view tools controlling ChronoLoom's semantic state. Use
