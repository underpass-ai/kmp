...
cargo run -p rehydration-testkit --bin longmemeval_kmp_evidence_builder -- \
  ...
  --output archive/artifacts/longmemeval-kmp/evidence-labels \
...
cargo run -p rehydration-testkit --bin longmemeval_kmp_embedding_candidates -- \
  --input /path/to/longmemeval_oracle.json \
  --output archive/artifacts/longmemeval-kmp/embedding-candidates \
...
cargo run -p rehydration-testkit --bin longmemeval_kmp_adapter -- \
  ...
  --output archive/artifacts/longmemeval-kmp/smoke \
  --limit 40
```
...
cargo run -p rehydration-testkit --bin longmemeval_kmp_adapter -- \
  --input /path/to/longmemeval_oracle.json \
  --output archive/artifacts/longmemeval-kmp/smoke \
  --evidence-labels archive/artifacts/longmemeval-kmp/evidence-labels/evidence_labels.jsonl \
  --limit 40
```
...
cargo run -p rehydration-testkit --bin longmemeval_kmp_runner -- \
  --artifacts archive/artifacts/longmemeval-kmp/smoke \
  --output archive/artifacts/longmemeval-kmp/run-smoke \
