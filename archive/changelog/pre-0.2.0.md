# Pre-0.2.0 changelog snapshot

> Historical snapshot archived on 2026-08-26. It preserves the detailed
> cumulative notes that remained under `Unreleased` after versions 0.1.13
> through 0.1.18 had shipped. The root changelog is the corrected release
> index; this file is evidence, not the current release ledger.
...
- Zero-configuration `kmp-mcp` sessions now mount the local viewer when the
  implicit backend is embedded. Backend selection previously defaulted to the
  embedded kernel while viewer selection checked only for an explicit
  `KMP_MCP_BACKEND=embedded`, silently skipping the viewer on the shortest
  install path.
- The repository now carries the complete Apache-2.0 license text, and both
  the container image and universal MCPB include `LICENSE`, `NOTICE` and the
  third-party notices instead of relying on a link back to the repository.
...
- **The public documentation now starts from the product KMP actually is.**
  The English README leads with local, private agent memory that preserves
  why; one reproducible terminal demo shows the real binary, and compact
  tables explain user interactions plus plugin, skill and MCP ownership. A
  new current spine separates embedded use, enterprise deployment,
  architecture, runbooks, development and research. All former documentation
  remains intact in one dated audit archive. CI rejects an unclassified
  current page, a former public tool name, or a tool count that disagrees with
  the binary.

- **KMP's research process is documented as part of the product.** Papers are
  working instruments that evolve with the implementation. The first,
  unpublished paper remains preserved as the initial complete expression of
  the multidimensional, temporal model and will be recovered into the active
  research surface as the evidence evolves.
...
- The twelve published crate pages, package descriptions and rustdocs now
  share KMP's local-first, evidence-backed product language; SQLite is the
  fresh-store path, redb remains an explicitly stamped compatibility path,
  and the gRPC wire namespace is named as backward compatibility rather than
  product identity. The quality gate builds all rustdocs with warnings denied.
