...
| Context | Owns | Today |
|:--|:--|:--|
| `contract` | What the server advertises and validates: the fifteen tool definitions, schema families, a minimal registry, argument validation | `protocol.rs` + `protocol/`, `args.rs` |
...
|:--|:--|:--|
| `protocol.rs` (1847) | Free functions returning `Value`; every tool definition in one file | `contract/`: one definition file per tool (fifteen), schema families, registry, validator adapter |
