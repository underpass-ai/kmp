...
| Labels (`kmp_curate mode: labels`) | A value under a key its about uses, for current facts that lack one: 6/6 right, in 3 samples. The one fact that fits no value (an SMS vendor decision against component values) was given one in every sample (0/1). | The agent does not read the label catalogue and every labelled fact to choose a value: it confirms a proposal and applies it with `kmp_relabel`. | About 1.4k tokens for 7 facts: $0.00006. | **Keep.** Right where a value fits. The agent must still reject a proposal for a fact that fits none. |
